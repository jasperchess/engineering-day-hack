...
export function useFileActivityLogger(options: UseFileActivityLoggerOptions): FileActivityLoggerHook {
  const { componentName, enableAutoTracking = true, trackPerformance = true } = options;
  ...
  const logUploadStart = useCallback((files: File[]) => {
    fileActivityLogger.logUploadStart(componentName, files);
  }, [componentName]);

  const logUploadProgress = useCallback((fileName: string, progress: number) => {
    fileActivityLogger.logUploadProgress(componentName, fileName, progress);
  }, [componentName]);

  const logUploadComplete = useCallback((file: any) => {
    fileActivityLogger.logUploadComplete(componentName, file);
  }, [componentName]);

  const logUploadError = useCallback((fileName: string, error: Error | string) => {
    fileActivityLogger.logUploadError(componentName, fileName, error);
  }, [componentName]);

  const logValidationError = useCallback((fileName: string, validationType: string, message: string) => {
    fileActivityLogger.logValidationError(componentName, fileName, validationType, message);
  }, [componentName]);

  const logFileSelect = useCallback((file: any) => {
    fileActivityLogger.logFileSelect(componentName, file);
  }, [componentName]);

  const logFileDownload = useCallback((file: any) => {
  ...
  const logFilePreview = useCallback((file: any, previewType: string) => {
    fileActivityLogger.logFilePreview(componentName, file, previewType);
  }, [componentName]);

  const logFileSort = useCallback((sortBy: string, sortOrder: string, fileCount: number) => {
    fileActivityLogger.logFileSort(componentName, sortBy, sortOrder, fileCount);
  }, [componentName]);

  const logFileFilter = useCallback((filterType: string, filterValue: any, resultCount: number) => {
    fileActivityLogger.logFileFilter(componentName, filterType, filterValue, resultCount);
  }, [componentName]);

  const logBatchOperation = useCallback((operation: string, fileIds: string[], success: boolean) => {
    fileActivityLogger.logBatchOperation(componentName, operation, fileIds, success);
  }, [componentName]);

  const logPerformanceMetric = useCallback((metric: string, value: number, unit: string) => {
    if (trackPerformance) {
      fileActivityLogger.logPerformanceMetric(componentName, metric, value, unit);
    }
  }, [componentName, trackPerformance]);

  const logActivity = useCallback((activity: FileActivity, options: any = {}) => {
    const logEntry = {
